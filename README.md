...
> 当前仓库存在 **两套内容并存**：
>
> - **新设计（v2，重构目标态）**：完整设计文档已沉淀到 [`specs/`](specs/) 目录，按"功能模块开发顺序"组织，作为后续实现的唯一参考。
> - **旧实现（v1，本 README 下文描述）**：FastAPI + SQLite + Vue3 一体化版本，代码继续可运行，文档保留作为对照。
>
> 重构期间，**新功能一律按 [`specs/`](specs/) 设计落地**，不再继续在旧实现上叠加。

---

## 重构设计目录（specs/）

> 入口文档：[`specs/0-总览/0.0-重构路线图.md`](specs/0-总览/0.0-重构路线图.md)（含全部 39 份设计文档索引、Phase 划分与依赖关系）

```text
specs/
├─ 0-总览/                    # 路线图 / 未决TODO清单 / 术语表
├─ 1-技术选型/                # 语言与 Agent 框架 / AgentScope+LlamaIndex 验证 / 中间件
├─ 2-数据模型/                # 题库主表 / Neo4j 图谱 / Prompt 表 / 归组表 / 面试表 / SM-2 表 /
│                            #   爬取扩展表 / 用户与笔记收藏表 / 审计与检索事件表
├─ 3-后台流水线/              # 爬取与抽取 / 图谱构建 / 相似归组 / 向量索引
├─ 4-检索召回/                # 召回策略骨架 / GraphRAG 查询模式
├─ 5-用户功能/                # Prompt-Playground / 题库浏览 / 闪卡复习 /
│                            #   模拟面试与出题 / 学习记录与报告 / 笔记收藏与错题本
├─ 6-前端模块/                # 用户侧 / 个人管理后台 / 后台监控 / 设计系统（部分 stub）
├─ 7-运维与可观测/            # 可观测性 / 监控仪表盘 / 告警与处置（stub）
└─ 8-治理与质量/              # 图谱健康 / 归组人工审核 / Prompt 评估 / 检索审计
```
...
| 维度 | v1 实现 | v2 设计 |
|---|---|---|
| **架构语言** | 单 FastAPI + 单 Agent 编排 | **Java（主业务/调度/API）+ Python（AI Worker）双栈**；Java 侧统一治理用户、任务、消息、缓存 |
| **Agent 框架** | hello-agents（自研 ReAct） | **AgentScope**（多 Agent 协作：Interviewer + Evaluator）+ **LlamaIndex**（RAG 与检索归口） |
| **存储分工** | SQLite + Neo4j + Qdrant 平铺 | **MySQL 主业务 + Neo4j 图谱（含向量索引）+ Redis 缓存**；Qdrant 仅在规模瓶颈时引入 |
| **题库去重** | 入库阶段强去重 | **链接去重（硬约束）+ 题目相似归组（展示层折叠，不删题）**两层独立解耦 |
| **GraphRAG** | 概念零散写入图谱 | **Domain（手动种子）→ Concept（LLM 抽取+向量去重）→ Question（COVERS_CONCEPT/RELATED_TO/VARIANT_OF）** 三层结构，图谱主路 + 向量兜底 |
| **Prompt 管理** | 散在代码中 | **prompt_versions 全版本化 + Playground 实验表 + 修正记录**，生产链路与实验链路完全隔离 |
| **复习调度** | 简单错题列表 | **SM-2 间隔重复 + 闪卡自评 + 面试评分双路写回**；自动错题本联动 |
| **用户系统** | 单用户 | **多用户（Phase 1 优先实施）**；笔记 / 收藏 / 错题本三表分开 |
| **可观测** | 后端日志 + 工具统计 | **OpenTelemetry trace_id 全链路贯穿 + retrieval_event 检索回放 + llm_call_audit 审计** |
| **治理** | 无 | 图谱健康（孤立/共现）、归组人工审核、Prompt 质量评估、检索 Hit Rate 评估 全部成体系 |

### 实施阶段总览（详见 0.0 路线图）

```
Phase 0: 基础设施搭建（Docker / DDL / 项目骨架）
Phase 1: 用户系统 + 爬取与抽取流水线 + Prompt Playground
Phase 2: GraphRAG 基础（概念建图 + 向量索引）
Phase 3: 题库浏览 + 笔记/收藏/错题本
Phase 4: 定时爬取（独立分支）
Phase 5: 闪卡复习（SM-2）
Phase 6: 纯出题练习
Phase 7: 全追问模拟面试（AgentScope 双 Agent）
Phase 8: 学习闭环（薄弱域反哺出题）
Phase 9: 生产上线 + 后期扩展
```

### 当前未决项

集中跟踪在 [`specs/0-总览/0.1-未决问题与TODO清单.md`](specs/0-总览/0.1-未决问题与TODO清单.md)，分三类：
- **架构取舍**（Jaeger vs Langfuse、Phoenix 嵌入与否、AgentScope Studio 集成方式 等）
- **数据/参数**（embedding 模型、SM-2 参数、爬取重试次数 等）
- **前端粒度**（6.x stub 待补全）

---

# InterviewExperienceCrawlerAgent（v1 现状，下文保留作为对照）
