...
def _get_warmup_targets():
    ...
    two_stage 模式下 Miner Stage1 用 MINER_REMOTE_*，需单独加入预热。
    """
    try:
        ...
        # two_stage：Stage1 端点链（本地 Ollama 才预热）
        if settings.miner_mode == "two_stage":
            for ep in getattr(settings, "miner_stage1_models", []) or []:
                b, m = (ep.get("base_url") or "").strip(), (ep.get("model") or "").strip()
                if b and m:
